...
Integration and Supervisor/add-on inventories are now refreshed **event-driven** in addition to their periodic safety refresh:

- adding, removing or updating a Home Assistant config entry schedules a new `integrations` payload;
- config-entry runtime state changes such as `setup_retry`, `setup_error`, `loaded` and unload transitions also schedule an update;
- Supervisor events schedule a fresh `addons` inventory, so add-on install/remove/start/stop/update changes are reflected much sooner;
- short debounce/settle delays coalesce bursts of Home Assistant events and avoid unnecessary MQTT traffic;
- the existing 5-minute periodic refresh remains as a fallback.

This means a resolved Control Room alert no longer normally needs to wait for the next 5-minute inventory cycle after an integration or add-on is removed/fixed.
